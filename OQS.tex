...
We also let the optimum solution be $\setI^* = \argmin_{|\setI| = k} \err (\setX, \setI)$ (if there are multiple, then choose one arbitrarily), and we let $\OPT_k = \err(\setX, \setI^*)$.
\paragraph{Discretization} 
One key observation is that we can \emph{discretize} the problem by noticing that there exists an optimal solution which places endpoints at input points:
\begin{lemma}
\label{lem:discrete}
...
With this in mind, it will be useful to rephrase the problem as follows. 
Given a set of initial candidate points $\mathcal{M}$, which is understood from context, let $T(k, N)$ be the optimal variance for a set of $N$ points $x_1, \ldots, x_N$, 
