...
The promise is simple and compelling: by reducing precision, we reduce computation, communication, and power costs by an order of magnitude, which can lead to an order-of-magnitude  end-to-end speedup. 
We examine this premise from a theoretical and practical perspective. We ask: 
is it possible to train models at end-to-end low precision with \emph{provable} guarantees?  can this lead to consistent order-of-magnitude speedups?

For linear models, the answer is yes. We develop a simple framework based on stochastic quantization, which is able to execute training at low precision with no bias, 
guaranteeing convergence. The main element of novelty is a technique for stochastic quantization of \emph{samples}, for which naive quantization would introduce  significant bias.  
We validate this framework across a range of applications, and show that it enables an FPGA prototype that is up to an order of magnitude faster than an optimized multi-threaded implementation using full 32bit precision.

We extend our framework through approximation to non-linear settings, such as SVM. 
We provide rigorous bias bounds, and show that in practice $8bits$ precision is always sufficient to converge to the correct solution.  
Interestingly, however, we notice that our framework does not always outperform the naive rounding approach. We discuss this negative result in detail. 

Further, we provide optimal algorithms for variance-optimal stochastic quantization, given fixed precision, and show that this can make a significant difference in a variety of settings. 
In particular, when training deep networks with quantized models, we achieve 
