...
\paragraph*{Refetching Heuristic}

We also develop one simple heuristic for SVM, which we will
describe in details in the full version of this paper.
For hinge loss $\sum_{k=1}^K \max(0, 1 - b_k \a_k^\top \x)$, the idea is to 
first load a low-precision version to 
calculate an upper bound and lower bound of $b_k \a_k^\top \x$.
If one cannot decide the sign of $1-b_k \a_k^\top \x$
with this, we {\em refetch} the high-precision data.
In practice, this heuristics works for 
8bits precision while only refetching $<5\%$
of the data.
...
empirical convergence rate. Because we need to send multiple samples if we use Chebyshev
polynomials, we actually cannot save memory-bandwith with our framework
compared to simple 8-bit deterministic rounding.
\begin{figure}[h]
\centering
    \begin{subfigure}[h]{.4\columnwidth}
...
\label{fig:optimal}
\end{figure}
\subsection{Extention to Deep Learning}
