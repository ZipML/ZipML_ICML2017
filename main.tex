...
There has recently been significant interest in  training and inference for machine-learning models at low precision. 
The promise is simple and compelling: by reducing precision, we reduce computation\footnote{\Ji: do we want to say ``inference'', which is versus to ``train'' later?}, communication, and power costs by an order of magnitude, which can lead to an order-of-magnitude  end-to-end speedup. 
We examine this premise from a theoretical and practical perspective. We ask: 
is it possible to \emph{train} models at end-to-end low precision with \emph{provable} guarantees?  can this lead to consistent order-of-magnitude speedups?

For linear models, the answer is yes. We develop a simple framework based on stochastic quantization, which is able to execute training at low precision with no bias, guaranteeing convergence. The main element of novelty is a technique for stochastic quantization of \emph{samples}, for which naive quantization would introduce significant bias. We validate this framework across a range of applications, and show that it enables an FPGA prototype that is up to an order of magnitude faster than an optimized multi-threaded implementation using full 32bit precision.

{\color{red}
For linear models, the answer is yes. We develop a simple framework based on three novel strategies: stochastic quantization, double sampling, and optimal quantization. It is able to execute training at low precision with no bias, guaranteeing convergence, while naive quantization would introduce significant bias. We validate this framework across a range of applications, and show that it enables an FPGA prototype that is up to an order of magnitude faster than an optimized multi-threaded implementation using full 32bit precision.
}

We extend our framework through approximation to non-linear settings, such as SVM. We provide rigorous bias bounds, and show that in practice $8bits$ precision is always sufficient to converge to the correct solution. Interestingly, however, we notice that our framework does not always outperform the naive rounding approach. We discuss this negative result in detail. 

{\color{red}
For non-linear models such as SVM, although the bias of using low precision data cannot be avoided, the proposed XXX can appropriately control the bias. We find that in practice $8bits$ precision is always sufficient to converge to the correct solution. Detailed discussion for some negative results are also provided.
}

Further, we provide optimal algorithms for variance-optimal stochastic quantization, given fixed precision, and show that this can make a significant difference in a variety of settings. In particular, when training deep networks with quantized models, we achieve higher accuracy than state-of-the-art BNNs. 
...
We validate these techniques across a range of applications, and show that they enable an FPGA prototype that is up to
an order of magnitude faster than a full 32bit precision implementation.
...
\label{fig:highlight}
\end{figure}
\footnote{\Ji: Graphs (a) and (d) in Figure 1 are clear enough. We may consider making Figure 1 larger.}
...
\textcolor{red}{JI: can we say something about how the discrete version approximates the real version?}
...
Recently, a lot of researchers have investigated low-precision deep neural networks, with different focuses. The first common way is to compress pre-trained networks and focus on small model size and fast inference.~\citet{vanhoucke2011improving} quantized model and activations to 8-bit fixed-point for fast SIMD operations on CPUs.~\citet{gong2014compressing} described how to use vector quantization to compress deep neural networks.~\citet{han2015deep} use k-means to group weights in to a few clusters to do quantization.~\citet{lin2016fixed} discussed how to allocate number of bits to use for each layer and saved 20\% in the model size compared to equal allocation.~\citet{kim2016bitwise} described Bitwise Neural Networks where all weights, activations and inputs are binary-values. These methods usually result in litte to no accuracy loss and enable doing inference on low-power mobile devices~\cite{kim2015compression,wu2016quantized}. On the other hand, there are another chunk of researches focusing on training with low-precison model and activations for fast training as well as fast inference. However, these methods will usually result in a loss in accuracy.~\citet{hubara2016quantized} introduced methods to train Binarized Neural Networks(BNN) and Quantized Neural Networks(QNN), by quantizing weights, activations and gradients to low-precision (eg. 1-bit). XNOR-net~\cite{rastegari2016xnor} also quantizes weights and activations to binary-values.~\citet{zhou2016dorefa} quantized gradients with low-precision on top of weights and activations. ~\citet{miyashita2016convolutional} proposed a method to represent weights and activations in log-domain.~\citet{li2016ternary} introduced ternary weight networks to quantize models so that the weights are constrained to 1, 0 and -1.~\citet{gupta2015deep} first decribed the idea of doing stochastic quantization instead of deterministic quantization.~\citet{seide20141} proposed a method to train deep neural networks using 1-bit gradients with errorection. Our approach is closer to the second method, our end-to-end framework would quantize input data, model and gradients to low-precision for faster computation and smaller memory bandwith and energy usage. Our task in argubaly harder than low-precision deep learning in the sense that our model is much smaller  
