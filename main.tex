...
\begin{abstract}
Recent work has shown that stochastic
rounding can be used in SGD to lower
the precision of gradient. However,
even for generalized linear models,
if applied to input samples or the model, 
it introduces bias because of the
non-linearty of the loss function.
In this paper, we present ZipML,
a framework for
low-precision data movement of
input samples and models. In ZipML,
we first represent arbitrary loss functions
with Chebyshev polynomials and then
develop sampling strategies for these
polynomials to get unbiased estimator for
the gradient. This allows us to 
lower the amount of data movement 
of input samples and models by
4$\times$-16$\times$ while
providing theoretical guarantee
on the bias (if any) of the training 
result. We further propose a 
data-optimal quantization strategy with 
dynamic programming, which further 
decrease data movement of input
samples by up to 2$\times$. ZipML
has been applied to applications
such as tomographic 
reconstruction and LASSO, and an
FPGA implementation provides 
up to 10$\times$ speed up over 
a full-precision implementation.
\end{abstract}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.4\textwidth]{Figures/RSHighlight}    
\caption{Result Highlights: For Linear Regression and LS-SVM
ZipML can lower the precision of {\em all} data movement to as low as 1-2 bits. 
For tomographic reconstruction, ZipML gets the same recovery 
quality with 3$\times$ less data movement. On FPGA, ZipML
can provide up to 10$\times$ speed up over 32bit implementation.}
\vspace{-2em}
\label{fig:highlight}
\end{figure}
