...
datasets we use. Due to space
limitations, we only report
(1) Synthetic 100 for regression, 
(2) gisette for classification,
and (3) CIFAR-10 for deep learning.
The full version of the paper contains a wide range of additional experiments.
...
$\alpha$ for the best performance of the full precision
implementation, and use the
same initial step size for 
our low precision 
implementation. 

We note that the low precision
implementation, as implied by
the theory and our experience, often favors smaller step size. 
In our experiments, we do not tune step sizes for the low precision 
...
on a state-of-the-art FPGA platform. We leave the detailed 
...
The motivating question of this paper was whether end-to-end low-precision data representation can enable efficient computation, 
...
In future work, we plan to further explore the potential of these techniques in the context of training and inference.  
