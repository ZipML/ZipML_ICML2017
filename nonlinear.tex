...
its relative value with respect to $0$ changes, which corresponds to having the \emph{wrong} label for the current sample.\footnote{The problem of training SVM with noisy labels has been previously considered previously, e.g.~\cite{Natarajan2013}, but in a setting where labels are corrupted uniformly at random. It is not hard to see that label corruptions are not uniform random in this case.}
...
The technical details are deferred to the supplementary material.
...
8-bit precision while only refetching $<5\%$ of the data.
