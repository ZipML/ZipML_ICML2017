...
In the remainder, we will assume that the processors have shared randomness.
...
Suppose furthermore that $\| \a_k \|_2^2 \leq 1$, and $\| \x \|_2^2 \leq R^2$, where $R \geq 1$.
 ...
 \[
 O\left( R^2 \frac{\log (1 / \tau) \log (n / \delta)}{\gamma^2} \right)
 \]
 bits.
\end{corollary}
\begin{proof}
...
\item
If $|c| \leq 2 \delta$, we refetch and compute the full (unquantized) gradient.
\item
Otherwise, we output $\g_k$.
\end{itemize}
...
There are two cases, depending on $c$.
Suppose we have $c \in [-2 \delta, 2 \delta]$.
Then, we have
\begin{align*}
\E [\hat{\g}_k \mid c \in [-2 \delta, 2 \delta]] &= \g'_k \; ,
\end{align*}
...
Finally, since $\Pr [c \not\in [-2 \delta, 2 \delta], \mathrm{failure}] \leq O(\delta)$, and $\| g_k \|_2 \leq 1$, (\ref{eq:l2-refetch}) implies that $\| \E [\hat{\g}_k] - \g'_k \|_2 \leq O(\delta)$.
